...
!(function (r, q, p) {
  function o(z) {
    var y,
      ...
      v,
      u,
      t = q.createElement("canvas"),
      s = t.getContext && t.getContext("2d"),
      b = String.fromCharCode;
    if (!s || !s.fillText) {
      return !1;
    }
    switch (((s.textBaseline = "top"), (s.font = "600 32px Arial"), z)) {
      case "flag":
        return (
          s.fillText(b(55356, 56806, 55356, 56826), 0, 0),
          !(t.toDataURL().length < 3000) &&
            (s.clearRect(0, 0, t.width, t.height),
            s.fillText(b(55356, 57331, 65039, 8205, 55356, 57096), 0, 0),
            (y = t.toDataURL()),
            s.clearRect(0, 0, t.width, t.height),
            s.fillText(b(55356, 57331, 55356, 57096), 0, 0),
            (x = t.toDataURL()),
            y !== x)
        );
      case "diversity":
        return (
          s.fillText(b(55356, 57221), 0, 0),
          (w = s.getImageData(16, 16, 1, 1).data),
          (v = w[0] + "," + w[1] + "," + w[2] + "," + w[3]),
          s.fillText(b(55356, 57221, 55356, 57343), 0, 0),
          (w = s.getImageData(16, 16, 1, 1).data),
          (u = w[0] + "," + w[1] + "," + w[2] + "," + w[3]),
          v !== u
        );
      case "simple":
        return (
          s.fillText(b(55357, 56835), 0, 0),
          0 !== s.getImageData(16, 16, 1, 1).data[0]
        );
      case "unicode8":
        return (
          s.fillText(b(55356, 57135), 0, 0),
          0 !== s.getImageData(16, 16, 1, 1).data[0]
        );
      case "unicode9":
        return (
          s.fillText(b(55358, 56631), 0, 0),
          0 !== s.getImageData(16, 16, 1, 1).data[0]
        );
    }
    return !1;
  ...
  function n(b) {
    ...
    (d.src = b),
      ...
      q.getElementsByTagName("head")[0].appendChild(d);
  }
  var m, l, k, j;
  for (
    j = Array("simple", "flag", "unicode8", "diversity", "unicode9"),
      p.supports = { everything: !0, everythingExceptFlag: !0 },
      k = 0;
    k < j.length;
    k++
  ) {
    (p.supports[j[k]] = o(j[k])),
      (p.supports.everything = p.supports.everything && p.supports[j[k]]),
      "flag" !== j[k] &&
        (p.supports.everythingExceptFlag =
          p.supports.everythingExceptFlag && p.supports[j[k]]);
  }
  (p.supports.everythingExceptFlag =
    p.supports.everythingExceptFlag && !p.supports.flag),
    (p.DOMReady = !1),
    (p.readyCallback = function () {
      p.DOMReady = !0;
    }),
    p.supports.everything ||
      ((l = function () {
        p.readyCallback();
      }),
      q.addEventListener
        ? (q.addEventListener("DOMContentLoaded", l, !1),
          r.addEventListener("load", l, !1))
        : (r.attachEvent("onload", l),
          q.attachEvent("onreadystatechange", function () {
            "complete" === q.readyState && p.readyCallback();
          })),
      (m = p.source || {}),
      m.concatemoji
        ? n(m.concatemoji)
        : m.wpemoji && m.twemoji && (n(m.twemoji), n(m.wpemoji)));
})(window, document, window._wpemojiSettings);
var tribe_events_linked_posts = {
  post_types: { tribe_venue: "venue", tribe_organizer: "organizer" },
};
...
jQuery(document).ready(function () {
  ajaxRevslider = function (d) {
    ...
    data.type = d.type;
    data.id = d.id;
    data.aspectratio = d.aspectratio;
    jQuery.ajax({
      type: "post",
      url: "#",
      dataType: "json",
  ...
  var b = function (c) {
    return jQuery(c.selector + " .rev_slider").revkill();
  };
...
(function () {
  ...
  var e = document.querySelectorAll('.mc4wp-form input[type="url"]');
  if (e && e.length > 0) {
  ...
  if (b.type !== "date") {
    var a = document.querySelectorAll('.mc4wp-form input[type="date"]');
    for (var d = 0; d < a.length; d++) {
      if (!a[d].placeholder) {
        a[d].placeholder = "YYYY-MM-DD";
      }
      if (!a[d].pattern) {
        a[d].pattern = "[0-9]{4}-(0[1-9]|1[012])-(0[1-9]|1[0-9]|2[0-9]|3[01])";
      }
    }
  }
})();
